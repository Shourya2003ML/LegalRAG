import chromadb
from configs.config import ALLOWED_COLLECTIONS

def get_collection_name_for_rag_type(rag_type: str)->str:
    """Here we create collections of the corpus"""
    collection_name = f"{rag_type.replace('-','_')}_collection"
    if collection_name not in ALLOWED_COLLECTIONS:
        raise ValueError(f"Invalid RAG type : {rag_type}")
    return collection_name

def list_existing_collections(persist_directory: str = "chromba_db") -> list:
    """List all existing collections in the persistent directory"""
    try:
